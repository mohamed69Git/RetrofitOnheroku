...
class AchatTicketController extends Controller
{
    public function acheter(Request $request)
    {
        ...
        if ($compte->solde > ($request->price * $request->quantite)) {
            if (($request->price == 100 || $request->price == (-100)) && ($compte->nbr_ticket_cent + ($request->quantite * ($request->price / 100)) >= 0)) {
                $compte->update([
            ...
            } else if ($request->price == 50 || $request->price == (-50) && ($compte->nbr_ticket_fifty + ($request->quantite * ($request->price / 50)) >= 0)) {
                $compte->update([
                    'solde' => $compte->solde - $request->price * $request->quantite,
